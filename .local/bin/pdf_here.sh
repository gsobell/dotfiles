...
## moded my gsobell to move pdf to working dir
## course staff my C/C++ course thinks .pptx is acceptable...
...
#       md2pdf "$directory""$file" -o "$ZA_CACHE_DIR""$pdffile"
      md2pdf "$directory""$file" -o "$pdffile"
    fi
  fi
  echo "Now we can open the file $ZA_CACHE_DIR$pdffile"
  # ...and after the conversion move the file to the original dir
  mv "$ZA_CACHE_DIR$pdffile" "$pdffile"
#   $READER "$pdffile"
fi
